...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActInitPreReadAll(void)
{
    ...
    } while(currBlockId > 1u); /* Never set the MultiBlock with ID 0 (PreReadAll = OFF, default) to SKIPPED */
...
#endif
    } while(currBlockId > lowestBlockId); /* blockId 0 does not need to be set to PENDING, already done */
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE)  NvM_InitializeBlockForReadAll(void)
{
    NvM_CurrentJob_t.JobBlockId_t++;
#if(NVM_PRE_READALL_API == STD_ON)
    while(NvM_IsBooleanBitfieldTrue(NvM_BlockDescriptorTable_at[NvM_CurrentJob_t.JobBlockId_t].Flags.SelectBlockForPreReadAllEnabled) == TRUE)
    {
        NvM_CurrentJob_t.JobBlockId_t++;
        if(NvM_CurrentJob_t.JobBlockId_t == NvM_NoOfBlockIds_t)
        {
