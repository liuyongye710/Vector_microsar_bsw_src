...
// Set the processing core to state 'Freeze'. In 'FREEZE' mode the code execution is stopped until a reset 
// (or a power off/on cycle) of the ECU occurs.
// This function never returns.
extern void CANoeEmuProcessor_CoreFreeze(void);

// Run a never ending loop that does nothing but consumes a lot of 'virtual' processor time.
// This function never returns.
extern void CANoeEmuProcessor_EndlessLoop(void);
...
// parameter comparand: the value used for comparison with the variable
// return value: the initial value of the variable
extern uint32 CANoeEmuProcessor_InterlockedCompareExchange(volatile uint32* destination, uint32 exchange, uint32 comparand);
