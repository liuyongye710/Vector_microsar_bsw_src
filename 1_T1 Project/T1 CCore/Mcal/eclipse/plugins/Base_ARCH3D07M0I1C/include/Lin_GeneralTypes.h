...
typedef enum
    ...
    LIN_CLASSIC_CS = 1U      /**< @brief Classic checksum model.*/
...
typedef enum
    {
    LIN_FRAMERESPONSE_TX = 0U,    /**< @brief Response is generated from
                                        this (master) node.*/
    LIN_FRAMERESPONSE_RX = 1U,     /**< @brief Response is generated from a
                                        remote slave node.*/
    LIN_FRAMERESPONSE_IGNORE = 2U      /**< @brief Response is generated from one
...
typedef enum
    {
    LIN_NOT_OK = 0U,     /**< @brief Development or production error
                                    occurred.*/
    LIN_TX_OK = 1U,          /**< @brief Successful transmission.*/
    LIN_TX_BUSY = 2U,        /**< @brief Ongoing transmission (Header or
                                    Response).*/
    LIN_TX_HEADER_ERROR = 3U,/**< @brief Erroneous header transmission such
                                    as:
                                     - Mismatch between sent and read
                                       back data
                                     - Identifier parity error
                                     - Physical bus error.*/
    LIN_TX_ERROR = 4U,       /**< @brief Erroneous transmission such as:
                                     - Mismatch between sent and read
                                       back data
                                     - Physical bus error.*/
    LIN_RX_OK = 5U,          /**< @brief Reception of correct response.*/
    LIN_RX_BUSY = 6U,        /**< @brief Ongoing reception:
                                    at least one response byte has
                                    been received, but the checksum
                                    byte has not been received.*/
    LIN_RX_ERROR = 7U,       /**< @brief Erroneous reception such as:
                                     - Framing error
                                     - Overrun error
                                     - Checksum error
                                     - Short response.*/
    LIN_RX_NO_RESPONSE = 8U, /**< @brief No response byte has been received
                                    so far.*/
                        /**< @brief This is a mess !!
                                   Frame status is mixed with channel
                                   status but i kept it here only
                                   because of LIN168.*/
    LIN_OPERATIONAL = 9U,    /**< @brief Normal operation;
                                     - The related LIN channel is ready
                                       to transmit next header
                                     - No data from previous frame
                                       available (e.g. after
                                       initialization).*/
    LIN_CH_SLEEP = 10U        /**< @brief Sleep mode operation;
...
typedef enum
    {
        ...
        LIN_ERR_RESP_STOPBIT = 1U, /**< @brief Framing error in response */
        
        LIN_ERR_RESP_CHKSUM = 2U, /**< @brief Checksum error */
        
        LIN_ERR_RESP_DATABIT = 3U, /**< @brief Monitoring error of transmitted data bit in response */
        
        LIN_ERR_NO_RESP = 4U, /**< @brief No response */
        
        LIN_ERR_INC_RESP = 5U /**< @brief Incomplete response */
