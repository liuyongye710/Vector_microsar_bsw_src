...
/**********************************************************************************************************************
*  Gpt_GetChannelTickRate
*********************************************************************************************************************/
/*! \brief      This function returns the scaled TickRate of a given channel to ensure backwards compatibility.
*  \param[in]  channel     The channel whose TickRate shall be returned
*  \return     Gpt_TickRateType     The TickRate of the channel
*  \pre        Gpt must be initialized
*  \note
*********************************************************************************************************************/
Gpt_TickRateType Gpt_GetChannelTickRate(Gpt_ChannelType channel);
...
*********************************************************************************************************************/
/*!
* - #10 Scale the tick rate of the channel with the default tick rate
*/
Gpt_TickRateType Gpt_GetChannelTickRate(Gpt_ChannelType channel)
{
    /* Needed to ensure compatibility with older versions */
    return (GPT_CHL(channel).TickRate / GPT_DEFAULT_CHANNEL_TICKRATE);
}
 ...
 *  Gpt_GetTimeElapsed
 *********************************************************************************************************************/
/*! \brief      This service returns the elapsed time of the respective channel in ticks.
...
FUNC(Gpt_ValueType, GPT_CODE) Gpt_GetTimeElapsed (Gpt_ChannelType channel)
{
  ...
  {
    Gpt_TickRateType TickRate = Gpt_GetChannelTickRate(channel);
    
    /* Get remaining time */
    TimeElapsed = (uint32)((Gpt_TickRateType)(VttGpt_GetRemainingTime(channel)) * TickRate);
 ...
 *********************************************************************************************************************/
/*! \brief      This service returns the remaining time of the respective channel in ticks
 *  \param[in]  channel                 Channel identifier
...
FUNC(Gpt_ValueType, GPT_CODE)  Gpt_GetTimeRemaining (Gpt_ChannelType channel)
{
  ...
  {    
      ...
      TimeRemaining = (uint32)((Gpt_TickRateType)(VttGpt_GetRemainingTime(channel)) * TickRate);
  }
 ...
 *  \param[in]  value                   Timeout period (in ticks) after which a notification shall occur.
                                        Value range is limited depending on the tick rate of the given channel.
 *  \return     void
 *  \pre        Module has to be initialized.
 *  \note
 *********************************************************************************************************************/
FUNC(void, GPT_CODE) Gpt_StartTimer (Gpt_ChannelType channel, Gpt_ValueType value)
{
  ...
  /* Scale the value with the tick rate of the channel (for CANoe only!!) 
     Necessary, so the notifications will arrive at the expected times */
  Gpt_TickRateType TickRate = Gpt_GetChannelTickRate(channel);
  Gpt_TickRateType ScaledValue = (Gpt_TickRateType)(value / TickRate);
  uint32 CanoeValue;

  if (ScaledValue >= 4294967295.0f /* Max value of Gpt_ValueType */)
  {
    CanoeValue = 4294967295uLL;
  }
  else if (ScaledValue < 1.0f)
  {
    /* If the division resulted in GPT_INVALID_TIMER_VALUE, only a fraction of the configured tick rate was given.
    In this case the best that can be done is to scale it up to a full tick (1 microsecond in CANoe's API).
    This is the maximum resolution we can offer. Anything below does not work! */
    CanoeValue = 1u;
  }
  else
  {
    CanoeValue = (Gpt_ValueType)(ScaledValue);
  }

  if (GPT_CHL(channel).ChannelMode == GPT_CH_MODE_CONTINUOUS)
  {
    VttGpt_StartTimer(channel, CanoeValue, VTTGPT_CH_MODE_CONTINUOUS);
  }
  else
  {
    VttGpt_StartTimer(channel, CanoeValue, VTTGPT_CH_MODE_ONESHOT);
  }
  return;
