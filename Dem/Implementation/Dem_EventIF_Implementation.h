...
# endif
  {
      ...
      || ((Dem_Cfg_IsObdIIExclusivelySupportedInVariant() == TRUE) && (EventContext->QualifyEvent == TRUE)))
    {
      ...
      if ((EventContext->EventEffects & DEM_CFG_TRIGGER_CONFIRMED) != 0u)
      {
        ...
        if (Dem_GlobalDiagnostics_Readiness_IsReadinessCalculationSupported() == TRUE)
        {
          if (Dem_Readiness_Completion_TestStaticallyCalculatedCompletionStates(Dem_Cfg_EventReadinessGroup(EventContext->EventId), DEM_COMPLETION_PIDCONTEXT_PID01_AND_PIDF501) == FALSE)
          {
            Dem_Readiness_Completion_SetReadinessGroupCompleted(TRUE, Dem_Cfg_EventReadinessGroup(EventContext->EventId));     /* SBSW_DEM_POINTER_FORWARD_GLOBAL */
          }
        }
      }
